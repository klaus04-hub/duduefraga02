...
WEBHOOK_BASE_URL = webhook_url or "https://placeholder.railway.app"
WEBHOOK_PATH     = "/telegram"

# ================= VALIDAÇÃO =================
if not TELEGRAM_TOKEN:
    raise RuntimeError("❌ Configure a variável TELEGRAM_TOKEN no Railway")
if not GROK_API_KEY:
    raise RuntimeError("❌ Configure a variável GROK_API_KEY no Railway")
...
def build_prompt():
    return (
        "Você é Maya, uma assistente amigável e descontraída.\n"
...
class Grok:
    async def reply(self, uid, text, image_base64=None):
        ...
        # Monta o conteúdo do usuário (texto e/ou imagem)
        if image_base64:
            user_content = []
            if text:
